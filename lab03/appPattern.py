...
class Memento:
    def __init__(self, bird_y, velocity, pipes, score):
        self.bird_y = bird_y
        self.velocity = velocity
        self.pipes = [p.copy() for p in pipes]
        self.score = score
...
class Game:
    ...
    def create_memento(self):
        return Memento(self.bird_y, self.velocity, self.pipes, self.score)

    def set_memento(self, memento):
        self.bird_y = memento.bird_y
        self.velocity = memento.velocity
        self.pipes = [p.copy() for p in memento.pipes]
...
class SaveManager:
    ...
    def save(self, slot, memento):
        self.slots[slot] = memento
...
while True:
        ...
        if event.type == pygame.KEYDOWN:
            ...
            if state == "game" and not game.game_over:
                ...
                if event.key == pygame.K_1:
                    saves.save(0, game.create_memento())
                    message = "Сохранено!"
                    message_timer = MESSAGE_DURATION
                if event.key == pygame.K_2:
                    saves.save(1, game.create_memento())
                    message = "Сохранено!"
                    message_timer = MESSAGE_DURATION
                if event.key == pygame.K_3:
                    saves.save(2, game.create_memento())
                    message = "Сохранено!"
                    message_timer = MESSAGE_DURATION

                # загрузить
                if event.key == pygame.K_q and saves.load(0):
                    game.set_memento(saves.load(0))
                    message = "Загружено!"
                    message_timer = MESSAGE_DURATION
                if event.key == pygame.K_w and saves.load(1):
                    game.set_memento(saves.load(1))
                    message = "Загружено!"
                    message_timer = MESSAGE_DURATION
                if event.key == pygame.K_e and saves.load(2):
                    game.set_memento(saves.load(2))
                    message = "Загружено!"
                    message_timer = MESSAGE_DURATION
        ...
        if event.type == pygame.MOUSEBUTTONDOWN:
            ...
            if state == "menu":
                ...
                if continue_btn.is_clicked(pos):
                    ...
                    latest_slot = None
                    latest_time = None
                    for i, m in enumerate(saves.slots):
                        if m is not None:
                            if latest_time is None or m.time > latest_time:
                                latest_time = m.time
                                latest_slot = i
                    if latest_slot is not None:
                        game.set_memento(saves.load(latest_slot))
                        message = "Загружено!"
                        message_timer = MESSAGE_DURATION
                        state = "game"
            ...
            # ---------- МЕНЮ СОХРАНЕНИЯ ----------
                ...
                for i, btn in enumerate(slot_buttons):
                    if btn.is_clicked(pos):
                        if event.button == 1:  # ЛКМ сохранить
                            saves.save(i, game.create_memento())
                            message = "Сохранено!"
                            message_timer = MESSAGE_DURATION
    ...
    if state == "menu":
        ...
        screen.blit(hint1, (10, 40))  # первая строка
        screen.blit(hint2, (10, 70))  # вторая строка чуть ниже
